...
def from_excel_click22(driver):
    count_tc = 0 ; df = common_selenium.zero_df()
    start_time = time.time() ; tc_name = "[TEST_999]"

    countkk = 0
    while True: 
        if countkk < 10:
            rowk = int(input("행 번호를 넣으세요 치세요 : "))
            dfi= DI2_call_excel(path_c("google.xlsx"), "lab")
            dfii = dfi.iloc[rowk-1:rowk,:]
            print(dfii)
